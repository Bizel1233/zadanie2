...
export default function DialogComponent({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, formikHelpers) => {
      ...
      if (editID !== undefined)
        return await formConfig.formikConfig.onSubmit(values, api, editID);
      ...
      if (values.items !== undefined)
        values.items = [...values.items, ...newItems];
    ...
    if (editID !== undefined && editID !== prevEditID) {
      ...
      formConfig.columns.map((column) => {
        if (column.type === "select") {
          if (column.select?.setSelected)
            column.select.setSelected(
              formConfig.formikConfig.initialValues[column.key]
            );
          if (column.select?.setSelectedItems)
            column.select.setSelectedItems(
              formConfig.formikConfig.initialValues[column.key]
            );
        }
      });
              ...
              >
                ...
                >
                  ...
                  <Button
                    variant="contained"
                    color="warning"
                    onClick={() => column.select?.setOpen(true)}
                  >
                    {column.select?.title}
                  </Button>
                </Box>
                ...
                <SelectTable
                  ...
                  itemsPerPageOptions={
                    column.select?.itemsPerPageOptions ?? [10, 25, 50, 100]
                  }
                  setItemsPerPage={(value) =>
                    column.select?.setItemsPerPage(value)
                  }
